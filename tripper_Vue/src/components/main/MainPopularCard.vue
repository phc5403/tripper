...
<template>
  <div class="row justify-content-center">
    <div class="col-lg-12 mt-5">
      <p class="fw-bold" style="font-size: 24px">현재 가장 인기 있는 관광지 TOP 3</p>
      ...
      <div class="row">
        <div class="col-md-4 position-relative" v-for="(popular, index) in populars" :key="index">
          <div class="p-3 text-center">
            ...
            />
            <p class="mt-3 fw-bold" style="font-size: 20px">{{ popular.title }}</p>
            <p class="mt-3" style="font-size: 16px">{{ popular.addr1 }}</p>
          </div>
        </div>
        <hr class="vertical dark mt-3" />
        <div v-show="selected !== null">
          <div>
            <p class="mt-3 fw-bold" style="font-size: 20px">✈ {{ title }}</p>
          </div>
