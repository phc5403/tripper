...
public class UserController {
	...
	public ResponseEntity<UserDto> getModifyUser(
			@PathVariable("user_id") @ApiParam(value = "얻어올 유저의 아이디", required = true) String user_id)
			throws Exception {
	...
	public ResponseEntity<String> delete(
			@PathVariable("user_id") @ApiParam(value = "삭제할 유저의 아이디", required = true) String user_id)
			throws Exception {
...
//            @ApiImplicitParam(name = "user_pwd", value = "비밀번호", required = true, dataType = "String", paramType = "query"), })
	...
	public ResponseEntity<Map<String, Object>> login(
		...
		try {
			...
			System.out.println(userService.loginUser(userDto));
			// MyBatis를 사용하여 로그인 쿼리 실행
//            UserDto userDto = userService.loginUser(loginParams);

//            System.out.println("loginParams -> " + ", " + loginUser.toString());
...
//            return exceptionHandling(e);
	...
	public ResponseEntity<Map<String, Object>> getInfo(
			@PathVariable("userId") @ApiParam(value = "인증할 회원의 아이디.", required = true) String user_id,
	...
	public ResponseEntity<?> removeToken(
			@PathVariable("userId") @ApiParam(value = "로그아웃할 회원의 아이디.", required = true) String user_id) {
		Map<String, Object> resultMap = new HashMap<>();
		HttpStatus status = HttpStatus.ACCEPTED;
		try {
			// 삭제 요청
...
//    }
	...
	@ApiResponses({ @ApiResponse(code = 200, message = "비밀번호 찾기 성공"), @ApiResponse(code = 404, message = "비밀번호 찾기 실패"),
			@ApiResponse(code = 500, message = "서버 오류") })
//    @ApiImplicitParams({
